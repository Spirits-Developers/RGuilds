...
public class TopPointsSubCommand {
    private final String subCommandSection = "Command.Guild.SubCommands.GuildTopPoints.";
    public TopPointsSubCommand(ProxiedPlayer player, String[] args) {
        if (args.length != 1) {
            TextHelper.sendPrefixedMessage(player, LanguageManager.getMsg(subCommandSection + "Usage"));
            return;
        }
        HashMap<Guild, Integer> topKills = Core.getInstance().getGuildManager().getTop(TopType.POINTS);
